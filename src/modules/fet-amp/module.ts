...
function render(host: HTMLElement) {
  ...
  for (const w of [device, config, IDQ, Kn, IDSS, Vp, RD, RS, RL, Rs, RG, RSbyp]) host.appendChild(w.el);
  ...
  const update = () => {
    ...
    if (dev === 'MOSFET') { params.Kn_mAV2 = Kn.value(); }
  ...
  for (const w of [device, config, IDQ, Kn, IDSS, Vp, RD, RS, RL, Rs, RG, RSbyp]) {
