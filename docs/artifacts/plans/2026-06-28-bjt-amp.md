...
Run: `npm test`
Expected: PASS — both CE tests green within tolerance. If `Av` is off, double-check the sign of the gm·vpi term in `solveCE` (collector voltage should go negative when vb goes positive) and the `gRE`/`gRs` guards (bypassed RE and Rs=0 must use `G_IDEAL`, not `1/0`).
...
Run: `npm test`
Expected: PASS — `solveCB` and `solveCC` were implemented in Task 2; this verifies them against the spec reference values. If CB Av is negative, the gm·vpi sign in `solveCB` is flipped (should be `vc = +gm·ve·RL_eff` because vpi = -ve and current source pulls c→e, so positive ve pushes vc positive). If CC Av or Zout drift, confirm `solveCC`'s `Zout` uses `(rpi+Rs)/(β+1)` (not `1/gm`) since the test has Rs=1.
